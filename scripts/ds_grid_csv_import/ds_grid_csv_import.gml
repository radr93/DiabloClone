...
Syntax:
ds_grid_csv_import(arg0,arg1,arg2,arg3,arg4)

CAUTION: Please ensure your files are in UTF-8 encoding.

Arguments:
arg0 = [file_path]			- string   Filename for the source UTF-8 CSV file
arg1 = [force_strings]		- bool     Whether to force all cells to be a string. Defaults to false
arg2 = [cell_delimiter]	- string   The delimiter used to separate cells. Defaults to a comma
arg3 = [string_delimiter]	- string   The delimiter used to define strings in the CSV file. Defaults to a double-quote
arg4 = [mac_newline]		- bool     Newline compatibility mode for Mac (0xA). Defaults to Windows standard newline (0xDA)

You may pass <undefined> to use the default value for optional arguments.  

Returns:
A ds grid that contains the data from the .csv file specified in argument 0.
...
if !( ( _bom_a == 239 ) and ( _bom_b == 187 ) and ( _bom_c == 191 ) ) {
	//show_debug_message( "CAUTION: ds_grid_csv_import: " + _filename + ": CSV file might not be UTF-8 encoded (no BOM)" );
	buffer_seek( _buffer, buffer_seek_start, 0 );
} else {
	_size -= 3;
}
